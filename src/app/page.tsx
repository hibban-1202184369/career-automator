        ...
        <div className="bg-slate-950 rounded-lg border border-slate-800 p-6 shadow-xl">
          ...
          {activeTab === 'config' && (
            <form onSubmit={handleSaveConfig} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-2">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-slate-950 border border-slate-700 rounded px-3 py-2 text-slate-200 focus:outline-none focus:border-blue-500 font-bold"
                      />
                      <p className="text-xs text-slate-400 mt-1">Maksimal {config.limitGlints || 80} lowongan di Glints.</p>
                    </div>
                    <div>
                      ...
                      />
                      <p className="text-xs text-slate-400 mt-1">Maksimal {config.limitJobstreet || 75} lowongan di Jobstreet.</p>
                    </div>
                    <div>
                      ...
                      />
                      <p className="text-xs text-slate-400 mt-1">Maksimal {config.limitLinkedin || 50} lowongan di LinkedIn.</p>
                    </div>
                    <div>
                      ...
                      />
                      <p className="text-xs text-slate-400 mt-1">Maksimal {config.limitIndeed || 50} lowongan di Indeed.</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                />
                  <div className="mt-4">
                    <label className="block text-sm font-medium text-slate-200 mb-1">Gemini API Key <span className="text-slate-400 font-normal">(auto-jawab screening - Fable & Astra logic)</span></label>
                    <input type="password" value={(config as any).geminiApiKey || ''} onChange={(e) => setConfig({ ...config, geminiApiKey: e.target.value } as any)} className="w-full px-3 py-2 border border-slate-700/80 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 font-mono text-xs" placeholder="AIzaSy... (kosongkan jika pakai ENV)" />
                    <p className="text-xs text-slate-400 mt-1">Cukup isi sekali di Konfigurasi Bot. Tidak perlu setting gateway apapun.</p>
                  </div>
              </div>
                            ...
                            return (
                              <tr key={item.id} className="hover:bg-slate-900/40 transition">
                                ...
                                <td className="p-3 text-xs text-slate-400 max-w-xs truncate" title={item.options}>
                                  {item.options || <span className="text-slate-300 italic">-</span>}
                                </td>
                                <td className="p-3">
          ...
          {activeTab === 'logs' && (
            <div className="space-y-4">
              ...
              >
                {logs.length === 0 ? (
                  <span className="text-slate-300 italic">Belum ada aktivitas. Silakan jalankan bot.</span>
                ) : (
                  logs.map((log, index) => {
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fadeIn">
            <div className="bg-slate-900 border border-amber-500/50 rounded-xl max-w-lg w-full p-6 shadow-2xl space-y-4">
              ...
              <div className="bg-slate-950 border border-slate-800 rounded p-3 text-xs font-mono text-rose-300 max-h-36 overflow-y-auto break-all">
                <span className="text-slate-400 block mb-1 font-semibold">Detail Log Error:</span>
                {sheetsWarning.error}
              </div>
